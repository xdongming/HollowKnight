...
BATCH_SIZE = 12  # 每次给agent learn的数据数量，从replay memory随机里sample一批数据出来
...
def run_episode(hp, algorithm, agent, act_rmp_correct, move_rmp_correct, PASS_COUNT, paused, model):
    ...
    for i in range(1):
        if (len(move_rmp_correct) > MEMORY_WARMUP_SIZE and len(act_rmp_correct) > MEMORY_WARMUP_SIZE):
            ...
            # with open('./act_memory/memory_' + str(m) + '.txt', 'rb') as f:
            #     act_rmp_correct.buffer=pickle.load(f)
            # with open('./move_memory/memory_'+str(m)+'.txt','rb') as f:
            #     move_rmp_correct.buffer=pickle.load(f)
